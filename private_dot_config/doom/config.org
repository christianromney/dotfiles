...
- a :: Ask (selection, line, interactive, about)
- c :: Code (add, edit, improve, complete, message, review)
- i :: Improve (conciseness, grammar, wording)
- m :: Make (list, table, format)
- p :: Provider (model)
- P :: Proofread
- s :: Summarize (selection, webpage)
- t :: Translate (buffer, text, complete)
- w :: Write /based on context + instruction at point/
- x :: conteXt (file, buffer, selection)
...
*** TODO Coding with Claude Code :experimental:
#+begin_src elisp
...
(~say~). It's [[https://apple.stackexchange.com/questions/464984/how-do-i-access-macos-sonoma-personal-voice-from-command-line][possible]] to use Mac's Personal Voice feature with the (say) command on the
command-line with this workaround, however I've not yet found a way to
request Personal Voice authorization from Emacs using something like the plist key above.
