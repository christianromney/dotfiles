...
*** org-ai
**** Security
For speech-to-text to work, I need to give emacs access to the microphone. Emacs
doesn't know how to do this at the moment.
***** Development Support
The correct way to do this is to request the
[[https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_security_device_audio-input][com.apple.security.device.audio-input]] [[https://developer.apple.com/documentation/bundleresources/entitlements][Entitlement]].

The Apple Developer documentation says:

#+begin_quote
To add this entitlement to your app, first enable the Hardened Runtime
capability in Xcode, and then under Resource Access, select Audio Input.
#+end_quote

This approach would require modifying the build process to use this approach,
possibly based on a new build configuration switch e.g.
`--with-audio-input-entitlement`

***** Hacky, Dangerous Workaround?
Warning: *The utility below is not confirmed to work on Sonoma* 14.3.1.

First, one would need to disable [[https://developer.apple.com/documentation/security/disabling_and_enabling_system_integrity_protection/][System Integrity Protection]] (SIP).
One can check whether SIP is enabled at the CLI with: ~csrutil status~.

Then, one could use a utility like the [[https://github.com/DocSystem/tccutil][tccutil]] to modify the
[[https://www.rainforestqa.com/blog/macos-tcc-db-deep-dive][Apple Transparency, Consent and Control (TCC) database]] directly.

More info on TCC is available in [[https://eclecticlight.co/2023/02/09/should-you-reset-its-database-or-delete-it-the-woes-of-tcc/][this blog post]].

#+begin_example
cd ~/src/open
git clone https://github.com/DocSystem/tccutil
cd tccutil
sudo python3 ./tccutil.py --id org.gnu.Emacs -e --microphone
#+end_example
